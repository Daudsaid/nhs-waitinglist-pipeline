...
# Only load these columns — ignore any extra Spark-generated columns
EXPECTED_COLUMNS = [
...
def load_parquet_to_snowflake(cursor, conn):
    ...
    # Drop any unexpected columns generated by Spark from mismatched CSVs
    existing_cols = [c for c in EXPECTED_COLUMNS if c in df.columns]
    df = df[existing_cols]
    print(f"Using {len(existing_cols)} columns.")
